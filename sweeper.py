...
# values = [ [ [('flatten', None), ('fc', int(x)), ('fc', int(x/2)), ('fc', int(x/4))] ] for x in values] 
# values2 = [1, 2, 3]

# construct notes string based on sweep parameters
notes = "attention single shot. Sweeping " + sweep_arg + " and " + sweep_arg2 + "."
...
param_dict = {
    ...
    "circ_filename": "../../qcircml_code/data/circset_4qubits_9gates_depth4_onebatch/dataset.p", # filename of circuit collection

    # batch parameters
    "reshuffle_dataset": True, # reshuffle the dataset with new batch_size, loops, and train_percent
    "batch_size": 90,
    "loops": int(100),
    "train_percent": 0.7,

    # model parameters
    "action_size": 9, # number of actions the agent can take
    "model_type": ["attention"],# "rl", "attention"], # type of model to use
    ...
    "transpose": [False], #, True, False], # whether to transpose the image before feeding it into the model, must be same size as layer_lists
...
for value in values:
    if sweep_arg2 == "None":
        ...
        param_dict["loops"] = int(30 * 0.7 / value) #FIXME: comment this out later
